...
# ---------------------------------------------------------------------------
# Chainable PIO instruction expression.
#
# An instruction such as ``set(x, 7)`` returns a value that supports the
# fluent ``.side(...)``/``.delay(...)``/``[delay]`` modifiers in any order:
#
#     set(x, 7).side(0)[7]
#     out(pins, 1)[6]
#     nop().side(1)[6]
#
# Each modifier returns the same expression so it can be chained. ``Self``
# carries the concrete type through the chain. ``_PIOInstr`` is intentionally
# distinct from ``rp2._PIO_ASM_Program`` (the opaque assembled program).
# ---------------------------------------------------------------------------
class _PIOInstr(Protocol):
    """Chainable PIO instruction expression. Supports ``.side()``, ``.delay()`` and ``[delay]``."""

    def side(self, value: int, /) -> Self: ...
    def delay(self, value: int, /) -> Self: ...
...
if TYPE_CHECKING:
    ...
    def word(instr, label: Incomplete | None = ...) -> _PIOInstr:
        """rp2.PIO instruction.
    ...
    def jmp(condition, label: Incomplete | None = ...) -> _PIOInstr:
    ...
    def wait(polarity: int, src: int, index: int, /) -> _PIOInstr:
    ...
    def out(destination: int, bit_count: int) -> _PIOInstr:
    ...
    def push(value: int = ..., value2: int = ...) -> _PIOInstr:
    ...
    def pull(value: int = ..., value2: int = ...) -> _PIOInstr:
    ...
    def irq(mod, index: Incomplete | None = ...) -> _PIOInstr:
        """rp2.PIO instruction.
    ...
    def set(destination, data) -> _PIOInstr:
